 ...
 *
 */
public class HolAuctioneerAgent extends Agent implements IAuctioneerAgent {
	...
	/* (non-Javadoc)
	 * @see msclab01.votingauction_lab.AuctioneerAgent.IAuctioneerAgent#callBehaviour(java.lang.String)
	 */
	@Override
				...
				if (msg != null) {
					...
					try {
						...
						if (bidderMoney.containsKey(msg.getSender()) && proposal > 0) {
							...
							if (leaderName.isEmpty() && proposal == bid && bidderMoney.get(msg.getSender()).intValue() >= proposal) {
	...
	private class TimeoutHandler extends WakerBehaviour {
		...
		protected void onWake() {
			...
			if (leaderName.isEmpty() && (goodPrice.get(goodType).intValue() > bid)) {
